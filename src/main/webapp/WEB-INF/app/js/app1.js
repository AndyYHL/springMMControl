...
App.run(["$rootScope", "$state", "$stateParams",  '$window','$http', '$templateCache', function ($rootScope, $state, $stateParams, $window,$http,$templateCache) {
    ...
    var menuJson = '/api/conl/sys/findMenuList',
    ...
    $rootScope.searchForm = {extLimit: $rootScope.pagination,data: $rootScope.data,info:$rootScope.info};
     ...
     success(function(data) {
         menuData = data.data;
         getMenu(menuData,stateProvider);
     }).
     error(function(err) {
     //错误代码
     });
...
App.config(['$controllerProvider', '$compileProvider', '$filterProvider', '$provide',
    ...
    function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
        ...
        $urlRouterProvider.otherwise('/app/ngdialog');
        //
        // Application Routes
        // -----------------------------------
        $stateProvider
...
App.controller('SidebarController', ['$rootScope', '$scope', '$state', '$http', '$timeout', 'Utils',
    function($rootScope, $scope, $state, $http, $timeout, Utils){
        ...
        $scope.loadSidebarMenu = function() {
            ...
            $scope.pagination = {pagestart: 0, pagesize: 10, maxsize: 8, pageindex: 1, limitOptions:[10,20,50,100]};
            $scope.data = {plevel_id:0};
            $scope.info={timestamp:new Date().getTime()}
            $scope.searchForm = {extLimit: $scope.pagination,data: $scope.data,info:$scope.info};

            $http.post(menuURL, $scope.searchForm).
                success(function(data) {
...
App.controller("deviceCtrl", function($scope, $location, $http,$rootScope,$timeout,ngDialog) {
...
function getMenu(menuList,stateProvider) {
    for (var i=0;i<menuList.length;i++){
        var submenu = menuList[i].submenu;
        if(menuList[i].sref != "#"){
            var sref = menuList[i].sref
            var url = menuList[i].path;
            var title = menuList[i].text;
            var templateUrl = menuList[i].templateurl;
            var controller = menuList[i].ctrl;
            if(menuList[i].files != ""){
                //var files = menuList[i].files;
                stateProvider.state(sref, {
                    ...
                    resolve:{
                        loadMyFiles:function($ocLazyLoad, $location) {
                            var pathUrl = $location.path();
                            var path = $location.path();
                            var attr = getFiles(menuData,path);
                            if (!attr) {
                                return;
                            }
                            ...
                            if (attr.files.indexOf(";") > -1) {
                                var list = attr.files.split(";");
                                var count = list.length
                                for (var i = 0; i < count; i++) {
                                    var filePath = list[i];
                                    $ocLazyLoad.load(filePath);
                                }
                                return true;
                                //return  $ocLazyLoad.load(list[count-1]);
                            }
                            else {
